...
data "vault_generic_secret" "ec2_data" {
  path = "applications/${var.aws_account}-${var.aws_region}/chips-e2e-pilot/${var.application}/ec2"
}
...
data "aws_security_group" "chips_e2e_pilot_sg" {
  for_each = toset(var.chips_e2e_pilot_sg)
  filter {
    name   = "group-name"
    values = [each.value]
  }
